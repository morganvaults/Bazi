...
> **Why Bazi?** Traditional Bazi astrology tools are often opaque, closed-source, or lack standardization. **Bazi**aims to provide a transparent, modular, and AI-augmented framework for computational astrology. By combining deterministic astronomical calculations with GPT-4's narrative reasoning, we transform complex astrological data into actionable decision-support insights.

## Quick Start

```bash
# Basic chart
python main.py --date 2026-06-29 --time 08:00 --gender 男

# With OpenAI GPT-4 narrative interpretation
python main.py --date 2026-06-29 --ai
```

The `--ai` flag calls the **OpenAI GPT-4** model (`gpt-4o-mini` by default, use `--ai-model gpt-4o` for full GPT-4) to generate an expert-level natural-language reading on top of the local analysis. Requires `OPENAI_API_KEY` environment variable or `--ai-key` argument.
