...
Calendar & month branch — Year pillar boundary is Start of Spring (立春), not Lunar New Year. Month branches follow the 12 solar terms.
Five elements — Both simple and weighted counts. Weights: heavenly stem 1.0; earthly branch main qi 1.0, mid qi 0.6, residual qi 0.3; month branch extra-weighted.
Day master strength — Based on month branch, roots, stems, and interactions (combinations, clashes, harms).
Useful god (用神) — Three layers: suppression/support god, seasonal adjustment god, and mediation god.
Luck pillars (大运) — Yang male / Yin female go forward; Yin male / Yang female go backward. 3 days = 1 year, 1 day = 4 months.
Scoring — Chart structure score and domain scores (career, wealth, relationships) computed separately from annual luck activation score.
Tomb, void, and 12 growth stages — Granular analysis; not every 辰戌丑未 branch is treated as day master burial.
Combinations, purity, and annual interactions — Combination success evaluated against seasonal support, stems, roots, and clash conditions.
Output Modes
Flag	Description
...
C 级：无资料，先索取出生信息。
核心计算步骤
历法与月令 — 年柱以立春为界，月柱以十二节气为界，不以农历正月初一为界。
五行统计 — 同时输出简单五行和加权五行（天干 1.0，地支藏干主气 1.0 / 中气 0.6 / 余气 0.3，月令额外加权）。
日主强弱 — 看月令、通根、透干、生扶克泄耗以及合冲刑害。
用神喜忌 — 三层取用：扶抑用神 → 调候用神 → 通关用神。
大运 — 阳男阴女顺排，阴男阳女逆排；三天一岁，一天四个月。
评分 — 原局结构分与领域评分（事业财运婚恋）分开，运势兑现评分单独呈现。
墓库、空亡、十二长生 — 分层细判，不把所有辰戌丑未都作日主入墓。
合化清浊与运年交互 — 合化成败看化神是否得令、透干、有根；格局清浊看月令格神与混杂情况。
输出模式
参数	说明
